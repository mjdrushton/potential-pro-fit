...
SplitVariableKey = collections.namedtuple("SplitVariableKey", ["job", "evaluator", "record", "attr"])
Expression = collections.namedtuple("Expression", ["name", "expression", "weight", "expected_value"])
...
class FormulaMetaEvaluator(object):
  ...
  def _makeVariableDict(self, joblist):
    ...
    for name, splitkey in self.variables:
      ...
      if not job:
        raise FormulaMetaEvaluatorVariableException("No job found named: %s" % str(splitkey.job) )
      job = job[0]
      found = False
      for evalrecords in job.evaluatorRecords:
        for evalrecord in evalrecords:
          ...
          if evaluatorName == splitkey.evaluator and evalrecord.name == splitkey.record:
            found = True
            break
        if found:
          break
      if not found:
        raise FormulaMetaEvaluatorVariableException("No evaluator record found for evaluator name: %s and value name: %s" % (splitkey.evaluator, splitkey.record) )
  ...
  def __call__(self, joblist):
    ...
    for expression in self.expressionList:
      try:
        result = cexprtk.evaluate_expression(expression.expression, variableDict)
  ...
  def _splitVariables(variables):
    ...
    for variable in variables:
      ...
      if len(tokens) == 4:
        ...
        raise fittool.ConfigException("Wrong number of fields when parsing key for Formula meta-evaluator variable '%s': '%s'" % (variable.name, variable.key))
      else:
        attr = 'merit_value'

      if  attr not in ['merit_value', 'extracted_value', 'weight', 'expected_value']:
        raise fittool.ConfigException("Unknown field in '%s' variable definition: '%s' for key '%s" % (variable.name, attr,variable.key))
  ...
  def _parseExpect(k, expression):
    ...
    if len(tokens) > 2:
      raise fittool.ConfigException("Error when splitting Formula meta-evaluator formula into expected_value = expression pair. Got > 2 tokens. (Expression may contain more than one = sign): '%s' : %s" % (k,expression))
    elif len(tokens) == 2:
      ...
      try:
        ...
        raise fittool.ConfigException("Error when parsing expected_value for Formula meta-evaluator. Could not convert '%s' into a float. ('%s' : %s)" % (expect, k,expression))
    else:
      expect = None
    ...
    for k, v in cfgItems:
      if k.startswith('weight_'):
        ...
        try:
          ...
          raise fittool.ConfigException("Error parsing configuration for meta evaluator '%s'. Variable weight '%s' couldn't be parsed into float: %s" (name, wname, v))
        weights[wname] = v
