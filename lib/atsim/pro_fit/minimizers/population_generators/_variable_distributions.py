...
class Uniform_Variable_Distribution(Variable_Distribution):
    ...
    def _init_bounds(self, variable_label):
        ...
        if not bounds:
            msg = "The uniform variable distribution needs bounds. Non found for variable '{}'".format(
                variable_label
            )
        ...
        if not math.isfinite(lower_bound):
            msg = "The uniform variable distribution lower bound was not finite for variable '{}'".format(
                variable_label
            )
            raise VariableException(msg)

        if not math.isfinite(upper_bound):
            msg = "The uniform variable distribution upper bound was not finite for variable '{}'".format(
                variable_label
            )
            raise VariableException(msg)
