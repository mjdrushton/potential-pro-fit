...
class _NelderMeadMeritCallback(object):
  ...
  def __call__(self, meritVals, candidateJobList):
    self.currentIterationResults.append(MinimizerResults(meritVals, candidateJobList))
...
class _NelderMeadStepMonitor(object):
  ...
  def __init__(self, stepCallback, merit):
    ...
    self._logger.debug('Registering _NelderMeadMeritCallback with merit object')
    if merit.afterMerit != None:
      raise ExistingCallbackException("Merit object already has afterMerit registered.")
    afterMerit = _NelderMeadMeritCallback()
    self.merit.afterMerit = afterMerit
    self.afterMerit = afterMerit

  def __call__(self, x, fval):
    self._logger.debug('_NelderMeadStepMonitor called with params: %s and fval:%s' % (x,fval))
    ...
    if self.bestSolution is None:
      ...
      self._logger.debug('Iteration step has improved global best meritValue')
      self.bestSolution = currentBest
...
class _NelderMeadInner(object):
  ...
  def _getBounds(self):
    ...
    bounds = []
    allUnbounded = True
    for (k,v,f), bound in zip(self._initialVariables.flaggedVariablePairs, self._initialVariables.bounds):
      if f:
...
class NelderMeadMinimizer(object):
  ...
  def createFromConfig(variables, configitems):
    ...
    allowedfields = set(['value_tolerance', 'function_tolerance', 'max_iterations'])
    actualfields = set(configitems.keys())

    if actualfields and not actualfields.issubset(allowedfields):
      notallowed = actualfields - (allowedfields.intersection(actualfields))
      notallowed = ",".join(sorted(notallowed))
      raise ConfigException("Unknown fields for NelderMead minimizer: %s" % notallowed)

    try:
      value_tolerance = float(configitems.get('value_tolerance',0.0001))
    except ValueError:
      raise ConfigException("Minimizer NelderMead could not convert 'value_tolerance' into a float: %s" % configitems['value_tolerance'])

    try:
      function_tolerance = float(configitems.get('function_tolerance',0.0001))
    except ValueError:
    ...
    if configitems.has_key('max_iterations'):
      try:
        ...
        raise ConfigException("Minimizer NelderMead could not convert 'max_iterations' into an int: %s" % configitems['max_iterations'])
    else:
      max_iterations = None
    ...
    try:
      ...
      raise ConfigException("Mystic package not found. NelderMead minimizer relies on mystic, please install")
