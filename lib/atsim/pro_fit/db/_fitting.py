...
class Fitting(object):
    ...
    def _getCandidateID(self, iterationNumber, candidateNumber):
        ...
        cidQuery = sa.select([candidates.c.id]).where(
            sa.and_(
                candidates.c.iteration_number == iterationNumber,
                candidates.c.candidate_number == candidateNumber,
            )
        )
    ...
    :return int: Maximum iteration number in database"""
        ...
        select = sa.select(
            [
                sa.func.max(
                    metadata.tables["candidates"].c.iteration_number
                ).label("max_iteration")
            ]
        )
    ...
    .. code-block:: javascript

      {
       ...
       'merit_value' : best merit value
        ...
        query = sa.select(
            [sa.func.count(table.c.id), sa.func.avg(table.c.merit_value)]
        ).where(table.c.iteration_number == iterationNumber)
        results = self.engine.execute(query)
        num_candidates, mean = results.fetchone()

        # Calculate standard deviation
        query = sa.select([table.c.merit_value]).where(
            table.c.iteration_number == iterationNumber
        )
        results = self.engine.execute(query)
    ...
    """
        ...
        query = sa.select(
            ...
            sa.and_(
                variable_keys.c.variable_name == variables.c.variable_name,
                variables.c.candidate_id == cid,
            )
        )
    ...
    """
        ...
        query = sa.select(
            ...
            from_obj=[
                sa.sql.expression.Join(
                    evaluated, jobs, evaluated.c.job_id == jobs.c.id
                ).outerjoin(
                    evaluatorerror,
                    evaluated.c.evaluatorerror_id == evaluatorerror.c.id,
                )
            ],
        )
