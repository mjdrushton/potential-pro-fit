...
class _RangeDiscoverIterator(object):
    ...
    def _secondStep(self, x, y):
        ...
        if xdir != 0 and ydir == 0:
            ...
            raise RangeException(
                "Could not identify major and minor axes. The major axis should remain the same whilst the other varies."
            )
    ...
    def _nextStep(self, x, y):
        ...
        if xdir == 0 and not self._xIsMajor:
            raise RangeException(
                "X-axis is not the major axis but x-value did not increase for current table row"
            )

        if ydir == 0 and self._xIsMajor:
            raise RangeException(
                "Y-axis is not the major axis but x-value did not increase for current table row"
            )
        ...
        if xdir != 0 and self._xdir is None:
            ...
            if x != self.x_range[0]:
                raise RangeException(
                    "X-axis reset. Was expecting a value of %f but found %f"
                    % (self.x_range[0], x)
                )
            ...
            if self._xcomplete:
                ...
                if x != self.x_range[self._i]:
                    raise RangeException(
                        "X-axis unexpected x value found. Expected %f but found %f"
                        % (self.x_range[self._i], x)
                    )
            else:
                self.x_range.append(x)
        ...
        if ydir != 0 and self._ydir is None:
            ...
            if y != self.y_range[0]:
                raise RangeException(
                    "Y-axis reset. Was expecting a value of %f but found %f"
                    % (self.y_range[0], y)
                )
            ...
            if self._ycomplete:
                ...
                if y != self.y_range[self._j]:
                    raise RangeException(
                        "X-axis unexpected y value found. Expected %f but found %f"
                        % (self.y_range[self._i], y)
                    )
            else:
                self.y_range.append(y)
...
def _populateArrays(
    table, xcolumn, ycolumn, zcolumn, missingValues, transposeZ=False
):
    rowkeys = next(table)
...
def serializeTableForR(
    table, outfile, xcolumn, ycolumn, zcolumn, missingValues=None
):
  ...
  * `z_name` : string containing value of `zcolumn` parameter passed to this function.
  ...
  :param missingValues: None values will be replaced by the value of this argument if provided."""
    ...
    x_range, y_range, zvals = _populateArrays(
        table, xcolumn, ycolumn, zcolumn, missingValues, transposeZ=True
    )

    outfile.write(
        "structure(list(x_name='%s',y_name='%s',z_name='%s',"
...
def serializeTableForGNUPlot(
  ...
  :param missingValues: None values will be replaced by the value of this argument if provided."""
    ...
    x_range, y_range, zvals = _populateArrays(
        table, xcolumn, ycolumn, zcolumn, missingValues
    )
