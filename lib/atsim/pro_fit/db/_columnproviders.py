...
class _RunningFilterColumnProvider(object):
    ...
    def __call__(self, iterationNumber, candidateNumber, rowDict):
        return [
            (
                self._columnName,
                self._RunningFilter(rowDict[self._primaryColumnKey]),
            )
        ]
...
class _VariablesColumnProvider(object):
  ...
  Column-label has format:
    ...
    @return VARIABLE_NAME"""
        ...
        query = sa.select([t.c.variable_name]).where(
            t.c.variable_name == variableName
        )
        with contextlib.closing(self.conn.execute(query)) as results:
            if not results.fetchone():
                raise KeyError(
                    "Variable not defined in database:" + variableName
                )
        return variableName
...
class _EvaluatorColumnProvider(object):
  ...
  * **Where:**
    ...
    _labelSplitRegex = re.compile(
        r"^evaluator:(?P<jobName>.*?):(?P<evaluatorName>.*):(?P<valueName>.*):(?P<valueType>merit_value|extracted_value|percent_difference)$"
    )
    ...
    def _splitColumnLabel(columnLabel):
        ...
        if not match:
            raise KeyError(
                "Bad evaluator column definition: '%s'" % columnLabel
            )
    ...
    def _performQuery(self, conn, tempMeta):
        ...
        if not (valueType in allowedValues):
            raise KeyError(
                ...
                (str(allowedValues), self.columnLabel),
            )
        ...
        extracols, self.rowpostprocess = {
            "merit_value": (
                [te.c.merit_value],
                operator.itemgetter("merit_value"),
            ),
            "extracted_value": (
                [te.c.extracted_value],
                operator.itemgetter("extracted_value"),
            ),
            "percent_difference": (
                [te.c.extracted_value, te.c.expected_value],
    ...
    :return list: List of column keys."""
        ...
        query = (
            sa.select([jt.c.job_name, et.c.evaluator_name, et.c.value_name])
            .distinct()
            .where(sa.and_(et.c.job_id == jt.c.id))
        )
        ...
        for row in results.fetchall():
            ...
            key = (
                "evaluator:%(job_name)s:%(evaluator_name)s:%(value_name)s"
                % rowdict
            )
...
class _StatColumnProvider(object):
  ...
  then applying different statistical calculations to it."""
    ...
    def __call__(self, iterationNumber, candidateNumber, rowDict):
        ...
        whichcol = [
            i for (i, cn) in enumerate(results.keys()) if cn == self.columnKey
        ][0]
