...
from ._exceptions import (
    BatchAlreadyFinishedException,
    BatchKilledException,
    BatchDirectoryLockException,
)
        ...
        def callback(exception):
            if not exception is None:
                self._logger.warning(
                    "Could not start batch %s, directory lock failed: %s",
                    self.name,
                )
    ...
    def monitorJobs(self):
        ...
        while self.areJobsRunning() and not self.killedEvent.is_set():
            try:
                ...
                self._logger.debug(
                    "monitorJobs job remove - %d jobs remaining",
                    len(self.extantJobs),
                )
        ...
        if exception:
            try:
                ...
                self._logger.getChild("batchKilled").warning(
                    "Batch was killed %s" % self.batch.name
                )
            except:
                self._logger.exception(
                    "Batch finished with exception: %s" % self.batch.name
                )

        if self.dirLocked:
            unlockEvent = self.batch.parentRunner.unlockPath(
                self.batch.remoteBatchDir
            )
        else:
