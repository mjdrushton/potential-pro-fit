...
class InnerRemoteRunner(BaseRemoteRunner):
  ...
  has a much more extensive interface (required by the RunnerBatch)"""
    ...
    """
        ...
        super(InnerRemoteRunner, self).__init__(
    ...
    """
        channel = RunChannel(
            self._gw, "%s-Run" % self.name, nprocesses=self._nprocesses
        )
        return channel
...
class RemoteRunner(object):
    ...
    """
        self._inner = InnerRemoteRunner(
            name, url, nprocesses, identityfile, extra_ssh_options, do_cleanup
        )
    ...
    def createFromConfig(runnerName, fitRootPath, cfgitems):
        ...
        options = InnerRemoteRunner.parseConfig(
        ...
        try:
            ...
            raise ConfigException(
                "Could not convert nprocesses configuration item into an integer"
            )
