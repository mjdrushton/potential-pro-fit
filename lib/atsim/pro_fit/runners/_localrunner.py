...
class _CopyDirectory(object):
    ...
    def doCopy(self, non_blocking=False):
        ...
        logger.debug(
            "Copying files from '%s' to '%s'.", self.source_path, self.dest_path
        )
...
class InnerLocalRunner(BaseRemoteRunner):
  ...
  has a much more extensive interface (required by the RunnerBatch)"""
    ...
    """
        channel = RunChannel(
            self._gw, "%s-Run" % self.name, nprocesses=nprocesses
        )
        return channel
    ...
    """
        return self._runClient.runCommand(
            handler.workingDirectory, handler.callback
        )
    ...
    def createDownloadDirectory(self, job):
        download = _CopyDirectoryDown(
            os.path.join(job.remotePath, "job_files"), job.outputPath
        )
    ...
    def createFromConfig(runnerName, fitRootPath, cfgitems):
        ...
        for k in cfgdict.keys():
            if not k in allowedkeywords:
                raise LocalRunner._makeException(
                    runnerName, "Unknown keyword '%s'" % k
                )
        ...
        try:
            ...
            raise LocalRunner._makeException(
                runnerName, "'nprocesses' configuration item not found"
            )
        ...
        try:
            ...
            raise LocalRunner._makeException(
                runnerName,
                "Could not convert 'nprocesses' configuration item into an integer. Value was = '%s'"
                % nprocesses,
            )
