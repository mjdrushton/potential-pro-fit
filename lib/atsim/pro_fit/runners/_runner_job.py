...
class _RunnerJobThread(object):
    ...
    def run(self):
        try:
            ...
            self._logger.getChild("run_exception").exception(
    ...
    def doLock(self):
        ...
        if not exc is None:
            self._logger.warning(
                "Could not start upload for %s, directory lock failed: %s",
                self.job,
                exc,
            )
            self.finishJob(exc)
    ...
    def doUpload(self):
        ...
        if self.killedEvent.is_set():
            ...
            if not exc is None:
                self._logger.info(
                    "Upload finished for job %s, with exception: %s",
                    self.job,
                    exc,
                )
                self.finishJob(exc)
                ...
                self._logger.debug(
                    "Upload finished successfully for %s", self.job
                )
        ...
        if self.killedEvent.is_set():
            ...
            if not exc is None:
                try:
                    ...
                    self._logger.warning(
                        "Job execution finished (finishJobRun) for %s, with exception: %s",
                        self.job,
                        exc,
                    )
                ...
                self._logger.info(
                    "Job execution finished successfully (finishJobRun) for %s",
                    self.job,
                )
    ...
    def doDownload(self):
        ...
        if self.killedEvent.is_set():
            ...
            if not exc is None:
                self._logger.warning(
                    "Download finished for job %s, with exception: %s",
                    self.job,
                    exc,
                )
                self.finishJob(exc)
                ...
                self._logger.debug(
                    "Download finished successfully for %s", self.job
                )
        ...
        try:
            ...
            if exception is None:
                ...
                try:
                    ...
                    self._logger.warning(
                        "%s finished with non-zero exit status: %s",
                        self.job,
                        exception,
                    )
                except:
            ...
            if not self.exception is None:
                self._logger.getChild("run_exception").warning(
                    "%s finished with exception: %s",
                    self.job,
...
class RunnerJob(object):
    ...
    def start(self):
        self._logger.info(
            "Starting job %s from batch %s"
            % (self.jobid, self.parentBatch.name)
        )
        self._logger.debug(
            "Job %s from batch %s has properties, sourcePath = '%s', remotePath = '%s' and outputPath = '%s'"
            % (
                self.jobid,
    ...
    def _lockDirectory(self):
        self._logger.debug(
            "Locking directory for job %s from batch %s"
            % (self.jobid, self.parentBatch.name)
        )
    ...
    def _startUpload(self):
        ...
        finishEvent, uploadDirectory = self.parentBatch.createUploadDirectory(
            self
        )
    ...
    def _startDownload(self):
        ...
        finishEvent, downloadDirectory = self.parentBatch.createDownloadDirectory(
            self
        )
