...
class QueueingSystemRunnerJobRecord(object):
  ...
  def submit(self):
    if not self._handlers:
      raise QueueingSystemRunnerJobRecordException("No jobs registered with QueueingSystemRunnerJobRecord")
    ...
    qs_client_record = self._qsclient.runJobs(joblist, callback, header_lines = self._header_include)
...
class QueueingSystemRunnerBatch(RunnerBatch):
  ...
  def __init__(self, parentRunner, remoteBatchDir, jobs, name, qs_client, header_include):
    self._logger = logging.getLogger(__name__).getChild("QueueingSystemRunnerBatch")
    super(QueueingSystemRunnerBatch, self).__init__(parentRunner, remoteBatchDir, jobs, name)
    self._header_include = header_include
  ...
  def resetQSJobRecord(self):
    ...
    self._logger.debug("Resetting QueueingSystemRunnerJobRecord for batch: '%s'. QueueingSystemRunnerJobRecord name  = '%s'", self.name, name)
    self.qs_jobrecord = QueueingSystemRunnerJobRecord(name, self.parentRunner.batch_size, self.qs_client, self._header_include)
    self._subBatchCount += 1

  def addJobToQSJobRecord(self, handler):
    self.qs_jobrecord.append(handler)
    self._logger.debug("Job added to sub-batch: '%s'", self.qs_jobrecord.name)
  ...
  def createDownloadDirectory(self, job):
    ...
    return super(QueueingSystemRunnerBatch, self).createDownloadDirectory(modified_job)
  ...
  def shouldSubmit(self):
    return self.qs_jobrecord.isFull or self._startJobRunCount >= len(self.jobs)

  def submitQSJobRecord(self):
    jr = self.qs_jobrecord
    self._logger.debug("Submitting sub-batch to queueing system: '%s' which contains %d jobs", jr.name, len(jr._handlers))
    self.resetQSJobRecord()
    jr.submit()
