    ...
    @return List of RMSEvaluatorRecord instances"""
        ...
        try:
            ...
            with open(statisFilename, "r") as statisfile:
                statisIterator = _dlpoly_parse.parseSTATIS(
                    statisfile, config, nptflag
                )
    ...
    def _extractValues(self, statisIterator):
        ...
        for row in statisIterator:
            if row["time"] >= self._startTime:
                rows.append(row)
        if row["time"] < self._startTime:
            raise DLPOLY_STATISEvaluatorException(
                "Maximum time in STATIS file < start_time"
            )
        return rows
        ...
        for k, v in list(cfgdict.items()):
            ...
            if len(tokens) == 2:
                try:
                    ...
                    raise ConfigException(
                        "'%s' record's weight not a valid float for Evaluator '%s': '%s'"
                        % (k, name, tokens[1])
                    )
            else:
                weight = 1.0
            ...
            try:
                ...
                raise ConfigException(
                    "Value '%s' record value not a valid float for Evaluator '%s': '%s'"
                    % (k, name, tokens[0])
                )
            ...
            if not ((k in allowedFields) or k.startswith("msd_")):
                raise ConfigException(
                    "Invalid field name: '%s' for Evaluator '%s'" % (k, name)
                )
