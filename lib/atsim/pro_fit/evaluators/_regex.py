...
class _RegexSubEvaluator(object):
    ...
    def __init__(self, name, configString, outputFilename):
        ...
        if not m:
            raise ConfigException(
                "Could not parse configuration for Regex evaluator, variable '%s': %s "
                % (name, configString)
            )
        ...
        try:
            with open(outputfilename, "r") as infile:
                for line in infile:
                    ...
                    if m:
                        _logger.debug(
                            "Found match using '%s' within line '%s'"
                            % (self._variableRegex.pattern, line)
                        )
            ...
            raise RegexEvaluatorException(
                "Regular expression did not match file contents or did not match sufficient times"
            )
        except Exception as e:
            return ErrorEvaluatorRecord(
                self.name, self.expectedValue, e, self.weight
            )
    ...
    def _configure(self, configstring):
        ...
        if not tokens:
            raise ConfigException(
                "When configuring Regex evaluator '%s', you must, at a minimum specify a regular expression and expected value"
                % self.name
            )
        ...
        try:
            ...
            raise ConfigException(
                "Could not parse expected value for variable '%s': '%s'"
                % (self.name, tokens[0])
            )
        ...
        try:
            ...
            raise ConfigException(
                "Could not parse weight for variable '%s':" % (self.name,)
            )
        ...
        if len(t) >= 1:
            try:
                ...
                raise ConfigException(
                    "Could not parse group number for variable '%s':"
                    % (self.name,)
                )
        ...
        if len(t) > 1:
            try:
                ...
                raise ConfigException(
                    "Could not parse file instance for variable '%s':"
                    % (self.name,)
                )
  ...
  from matched values"""
    ...
    def createFromConfig(name, jobpath, cfgitems):
        ...
        try:
            ...
            raise ConfigException(
                "Could not find 'filename' record in configuration for '%s' Regex evaluator."
                % name
            )
