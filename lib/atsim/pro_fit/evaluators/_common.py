...
class EvaluatorRecord(object):
    ...
    def __repr__(self):
        return (
            "EvaluatorRecord(name=%s, expectedValue=%s, extractedValue=%s, weight=%s, meritValue=%s, evaluatorName=%s)"
            % (
                self.name,
                self.expectedValue,
                self.extractedValue,
...
class FractionalDifferenceEvaluatorRecord(RMSEvaluatorRecord):
    ...
    def __init__(self, name, expectedValue, extractedValue, weight=1.0):
        RMSEvaluatorRecord.__init__(
            self, name, expectedValue, extractedValue, weight
        )
...
class ErrorEvaluatorRecord(EvaluatorRecord):
    ...
    def __init__(
        self, name, expectedValue, exception, weight=1.0, evaluatorName=None
    ):
        EvaluatorRecord.__init__(
            self,
            name,
    ...
    def __repr__(self):
        return (
            "ErrorEvaluatorRecord(name=%s, exception=%s, evaluatorName=%s)"
            % (self.name, self.exception, self.evaluatorName)
        )
