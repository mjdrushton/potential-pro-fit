...
def parseCONFIG(infile):
  ...
  def readcoord():
    ...
    atom = {'atmnam':atmnam, 'coord':pos, 'atmnum' : atmnum, 'index' : index}
    return atom
...
def parseSTATIS(infile, config = None, npt = False):
  ...
  return _extraFieldDictIterator(
      _parseSTATISInternal(infile),
      species,
      npt)

statisColumnKeys = ['engcns',
  'temp',
  'engcfg',
  'engsrp',
  'engcpe',
...
def _extraFieldDictIterator(statisIterator, speciesNames, nptFlag):
  ...
  extrafieldkeys.extend(['stressxx', 'stressxy', 'stressxz',
                         'stressyx', 'stressyy', 'stressyz',
                         'stresszx', 'stresszy', 'stresszz'])

  if nptFlag:
    extrafieldkeys.extend([ 'cella_x', 'cella_y', 'cella_z',
      'cellb_x', 'cellb_y', 'cellb_z',
      'cellc_x', 'cellc_y', 'cellc_z'])
