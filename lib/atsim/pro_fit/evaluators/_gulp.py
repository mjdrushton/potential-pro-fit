...
class GulpEvaluator(object):
  ...
  def createFromConfig(name, jobpath, cfgitems):
    ...
    try:
      ...
      raise ConfigException("'filename' record not found for Evaluator '%s'" % name)
    ...
    for k, v in cfgdict.iteritems():
      ...
      if not k in _allowedGulpFields:
        raise ConfigException("'%s' unknown key for Evaluator '%s'" % (k,name))
      ...
      try:
        ...
        raise ConfigException("'%s' record's expect not a valid float for Evaluator '%s': '%s' " % (k,name, tokens[0]))
