...
class RunnerController(object):
    ...
    def _registerHandlers(self):
        ...
        m.observers.uploaded = PropertySetHandler(
            self.widget.pb_upload, "current", int
        )
        m.observers.downloaded = PropertySetHandler(
            self.widget.pb_finished, "current", int
        )
        m.observers.running = PropertySetHandler(
            self.widget.pb_running, "current", int
        )
...
class RunnersController(object):
    ...
    def _registerHandlers(self):
        ...
        RunnerController(
            self.model.runner_overview, self.mainframe.jobs.totalProgressBars
        )
...
class ConsoleController(object):
  ...
  widgets in MainFrame"""
    ...
    """
        ...
        self._messagesController = MessagesController(
            self.model.messages, self.mainframe
        )
    ...
    def _registerHeaderHandlers(self):
        ...
        # Run Name
        self.model.observers.run_name = PropertySetHandler(
            mf.header, "run_name"
        )

        # Iteration numbers
        self.model.current_iteration.observers.iteration_number = PropertySetHandler(
            mf.header, "current_iteration_number"
        )
        self.model.best_iteration.observers.iteration_number = PropertySetHandler(
            mf.header, "best_iteration_number"
        )

        # Merit values
        self.model.current_iteration.observers.merit_value = PropertySetHandler(
            mf.header, "current_merit_value"
        )
        self.model.best_iteration.observers.merit_value = PropertySetHandler(
            mf.header, "best_merit_value"
        )
