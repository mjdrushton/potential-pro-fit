...
CurrentBestTuple = collections.namedtuple(
    "CurrentBestTuple", ["name", "current", "best"]
)
...
class Variables(urwid.WidgetWrap):
    def __init__(self):
        self._listboxModel = CurrentBestListWalker(
            [CurrentBestTuple("NA", "NA", "NA")]
        )
        super(Variables, self).__init__(self._makeWidgets())

    def _makeWidgets(self):
        variable_listbox = urwid.ListBox(self._listboxModel)
        variable_listbox_header = CurrentBestListWalker.makewidget(
            CurrentBestTuple("Name", "Current", "Best")
        )
        headed_variable_listbox = urwid.Pile(
            [("pack", variable_listbox_header), variable_listbox]
        )
