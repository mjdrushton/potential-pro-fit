...
class Header(urwid.WidgetWrap):
    ...
    def _makeWidgets(self):
        ...
        header = urwid.Columns(
            [
                ("weight", 2, urwid.Pile([title_label, self._run_name_label])),
                (
                    5,
                    urwid.Pile(
                        [
    ...
    current_iteration_number = property(
        _get_current_iteration_number, _set_current_iteration_number
    )
    ...
    best_iteration_number = property(
        _get_best_iteration_number, _set_best_iteration_number
    )
    ...
    current_merit_value = property(
        _get_current_merit_value, _set_current_merit_value
    )
