...
class ErrorDialog(urwid.WidgetWrap):
    ...
    def _buildContents(self):
        ...
        footer = urwid.Padding(
            self.button, align="center", width=("relative", 50)
        )
...
class MainFrame(urwid.WidgetPlaceholder):
    ...
    def _makeBody(self):
        ...
        container = urwid.Pile(
            [("weight", 1.0, boxed_variables), ("pack", boxed_jobs)]
        )
        return container
    ...
    def showErrorMessage(
        self, msg, callback, button_text="Exit", title="Error"
    ):
