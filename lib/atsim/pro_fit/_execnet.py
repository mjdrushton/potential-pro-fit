...
def makeExecnetConnectionSpec(
    username, host, port, identityfile=None, extraoptions=[]
):
    gwurl = "ssh="
    ...
    local_log.debug(
        "Execnet url: '%s' for username = '%s', host = '%s', port= '%s', identityfile = '%s'"
        % (gwurl, username, host, port, identityfile)
    )
