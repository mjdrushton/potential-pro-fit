...
function Evaluated(iteration_number, candidate_number)
{
   ...
   self.aoColumns = ko.observable([
	...
	self._selectionCols = { 'extracted_value' : null,
							'merit_value' : null,
	...
	//
	// Reading this property returns a list.
	// By comparison, a single value is accepted when writing to the property. This value will be removed from the selection set
	// if it exists in the list returned by selected_column_keys.
	self.selected_column_keys = ko.computed({
		read: function(){
			var tselect = self._table_selection();
			var retlist = [];
			...
			$.each(retlist, function(i,v){
				retlist[i] = 'evaluator:'+v.job+":"+v.evaluator_name+":"+v.value_name+":"+v.col_name;
			});
		...
		write : function(v){
			if (! v.match(/^evaluator:.*:.*:.*:.*$/))
			{
				return;
			}

			var key = v.replace(/^evaluator:/, "");
			var d = self._table_selection();
			delete d[key];
			self._table_selection(d);
		}
	});
...
function OverViewTable(current_iteration, best_iteration) {
	...
	// Method that returns true if the given columnLabel is a pinned evaluator or variable column.
	self.isPinnedColumn = function(columnLabel)
	{
		return self.enabled() && ((self.variable_columns().indexOf(columnLabel) != -1) || (self.evaluator_columns().indexOf(columnLabel) != -1)); 
	};

	// // Method that will remove columnLabel from the variable_columns or evaluator_columns list.
	self.unPinColumn = function(columnLabel)
	{
		if (! self.enabled())
		{
			return;
		...
		if (self.variable_columns().indexOf(columnLabel) != -1)
		{
			self.variable_columns(columnLabel);
		}
		else if (self.evaluator_columns().indexOf(columnLabel) != -1)
		{
			self.evaluator_columns(columnLabel);
		} 
	};
