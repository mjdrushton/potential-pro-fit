  ...
  */
{
	...
	self.setKeyRecords = function(keyRecords)
	{
		//Join
		var legenditems = self._contents
			.selectAll('.legend_item')
			.data(keyRecords, function(d){
				return d.class+':'+d.text;	
			});

		// Create legend items
		ent = legenditems.enter()
			.append('g')
		...
		var symcallback = function(d){
			return 'symbol_group series '+d.class;
		};

		// Update
		legenditems.selectAll('.symbol_group')
			.attr('class', symcallback);

		legenditems.selectAll('.label')
...
var setupPlot = function(model)
{
	...
	var series = new OverviewTableSeries(model, plotObj, xaxis, yaxis);
	var key = new Key(plotObj, []);

	//When selected plot column changes upate the key and axis title.
	ko.computed(function(){
		var y_title = model.plot_model.y_title();
		var keyRecords = [
			{class : 'series_current', text : y_title},
			{class : 'series_best',  text : 'Iteration with improved merit'}
		];
		key.setKeyRecords(keyRecords);
