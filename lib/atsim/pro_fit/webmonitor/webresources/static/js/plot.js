...
/** When instances of _DragOffsetInteraction are created,
  ...
  *	occurred.
  *
  *	@param ele DOM element that should fire dragOffset events when mouse drag happens above it
  */
{
	var self = this;
	self._ele = $(ele);
	...
	self.start = function(e, ui)
	{
		self._x = ui.offset.left;
		self._y = ui.offset.top;
		self._cursor = self._ele.css('cursor');
		self._ele.css('cursor' , 'move');
	};

	self.drag = function(e,ui)
	{
		var nx = ui.offset.left;
		var ny = ui.offset.top;
		var offset = {x : nx - self._x, y: ny - self._y};
		self._x = nx;
		self._y = ny;
	...
	{
		self._ele.css('cursor', self._cursor);
	};

	// self._ele.draggable(self);
};
  ...
  */
{
	...
	// ... plot area drag
	self._plotAreaDragOffsetEventGenerator = new _DragOffsetInteraction($(self._bg[0]));
	$(self._plotAreaDragOffsetEventGenerator).on('dragOffset', self._plotAreaDragHandler);
	// ... axis drag
	self._xAxisPlotAreaDragOffsetEventGenerator = new _DragOffsetInteraction($(self._series.getXAxis().getForegroundRectangle()[0]));
	$(self._xAxisPlotAreaDragOffsetEventGenerator).bind('dragOffset', {'axis' : self._series.getXAxis()}, self._axisDragHandler);

	self._yAxisPlotAreaDragOffsetEventGenerator = new _DragOffsetInteraction($(self._series.getYAxis().getForegroundRectangle()[0]));
