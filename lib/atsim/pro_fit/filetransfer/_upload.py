...
class UploadChannel(BaseChannel):
    ...
    _logger = logging.getLogger(
        "atsim.pro_fit.runners._file_transfer_client.UploadChannel"
    )
...
class UploadChannels(MultiChannel):
    ...
    _logger = logging.getLogger(
        "atsim.pro_fit.runners._file_transfer_client.UploadChannels"
    )
    ...
    def __init__(
        ...
        super(UploadChannels, self).__init__(
...
class UploadHandler(object):
    ...
    _logger = logging.getLogger(
        "atsim.pro_fit.runners._file_transfer_client.UploadHandler"
    )
    ...
    def transform_path(self, path):
        if not path.startswith(self.source_path):
            raise UploadHandlerException(
                "Path to be transformed did not start with source_path. Path: '%s', source_path: '%s'"
...
class UploadDirectory(object):
    ...
    def __init__(
        self, ulchannels, local_path, remote_path, upload_handler=None
    ):
    ...
    """
        ...
        log.info(
            "Starting upload id='%s' from local ('%s') to remote ('%s').",
            self.transaction_id,
            self.local_path,
            self.remote_path,
        )
...
class _UploadCallback(object):
    ...
    def _make_dest_path(self):
        ...
        request = self._build_msg(
            "MKDIRS", transid, remote_path=self.parent.remote_path
        )
    ...
    """
        ...
        msgdict = self._build_msg(
            "MKDIR", transid, mode=mode, remote_path=local_path
        )
