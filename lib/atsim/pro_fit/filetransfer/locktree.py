...
class LockTree(object):
    ...
    def _registered(self):
        VisitorState = collections.namedtuple(
            "VisitorTuple", ("path_components", "is_locked")
        )
        ...
        for node, state in self.rootnode.treeIterator(
            visitor, VisitorState(tuple(), None)
        ):
    ...
    def _splitpath(self, pathstring):
        if os.path.isabs(pathstring):
            ...
            if not pathstring.startswith(self.root):
                raise LockTreeException(
                    "Absolute path does not lie within root ('%s'): '%s'"
