...
_DirectoryRecord = collections.namedtuple(
    "_DirectoryRecord", ["transid", "path"]
)
...
class DownloadChannel(BaseChannel):
    ...
    _logger = logging.getLogger(
        "atsim.pro_fit.runners._file_transfer_client.DownloadChannel"
    )
...
class DownloadChannels(MultiChannel):
    ...
    _logger = logging.getLogger(
        "atsim.pro_fit.runners._file_transfer_client.DownloadChannels"
    )
    ...
    def __init__(
        ...
        super(DownloadChannels, self).__init__(
            execnet_gw, factory, num_channels, channel_id
        )
...
class DownloadHandler(object):
  ...
  wire messages into directories and files on disc"""
    ...
    _logger = logging.getLogger(
        "atsim.pro_fit.runners._file_transfer_client.DownloadHandler"
    )
    ...
    def writefile(self, msg):
        ...
        mode = msg["mode"]
        self._logger.debug(
            "writing file with path: '%s' and mode %o", local_path, mode
        )
...
class DownloadDirectory(object):
  ...
  allow directory hierarchies to be downloaded."""
    ...
    _logger = logging.getLogger(
        "atsim.pro_fit.runners._file_transfer_client.DownloadDirectory"
    )

    def __init__(
        self, dlchannels, remote_path, dest_path, download_handler=None
    ):
    ...
    """
        ...
        log.info(
            "Starting download id='%s' from remote ('%s') to local ('%s').",
            self.transaction_id,
            self.remote_path,
            self.dest_path,
        )
...
class _DownloadCallback(object):
    ...
    def _is_msg_relevant(self, msg):
        ...
        if not retval:
            self._logger.debug(
                "Callback ID: '%s'. Message ID doesn't match transaction_id and will not be processed. %s != %s ",
                self.parent.transaction_id,
                transid,
                self.parent.transaction_id,
            )
    ...
    def _process_queue_item(self, msg):
        self._logger.debug(
            "Callback ID: '%s'. Received message: '%s'",
            self.parent.transaction_id,
            msg,
        )
        if not self.enabled:
            self._logger.debug(
                "Callback ID: '%s'. Callback is not enabled, ignoring message.",
                self.parent.transaction_id,
            )
            return
        ...
        if mtype == "ERROR":
            if msg.get("error_code", None) == ("OSERROR", "LISTDIR"):
                self._logger.warning(
                    "Callback ID: '%s'. Could not list directory (skipping contents): '%s'",
                    self.parent.transaction_id,
                    msg.get("exc_msg", None),
                )
                dirid = msg.get("id")
                self._skip_dir(dirid)
            elif msg.get("error_code", None) == ("IOERROR", "FILEOPEN"):
                self._logger.warning(
                    "Callback ID: '%s'. Could not open file for reading (skipping): '%s'",
                    self.parent.transaction_id,
                    msg.get("exc_msg", None),
                )
                fileid = msg.get("id")
                ...
                self._error(
                    "Received 'ERROR' message from remote: %s" % msg["reason"]
                )
                return
    ...
    def _error(self, msg):
        try:
            self._logger.warning(
                "Callback ID: '%s'. Error: %s", self.parent.transaction_id, msg
            )
    ...
    def _isFinished(self):
        finished = (
            len(self.dir_q) == 0
            and len(self.dir_q_wait) == 0
            and len(self.file_q_wait) == 0
        )
        return finished
    ...
    def _list_dir_request(self, transid, remotepath):
        ...
        self._logger.getChild("_list_dir_request").debug(
            "Listing remote: '%s'", remotepath
        )
    ...
    def _download_file_request(self, transid, remotepath):
        ...
        self._logger.getChild("_download_file_request").debug(
            "Requesting file: '%s'", remotepath
        )
    ...
    This triggers the following:
      ...
      * check if we're finished and trigger event if we are"""
        ...
        if not transid in self.dir_q_wait:
            raise DirectoryDownloadException(
                "Unexpected LIST response received for '%s'", transid
            )
