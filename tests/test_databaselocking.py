...
def populatedatabase(dbfilename, pipe):
    try:
        ...
        initialVariables = (
            test_reporters.SQLiteReporterTestCase.getInitialVariables()
        )
        calculatedVariables = (
            test_reporters.SQLiteReporterTestCase.getCalculatedVariables()
        )
        reporter = SQLiteReporter(
            dbfilename, initialVariables, calculatedVariables
        )
...
class SQLReporterDatabaseLockTestCase(unittest.TestCase):
    ...
    successfully written to the database"""
        ...
        populateProcess = mp.Process(
            target=populatedatabase, args=(self.dbfilename, popPipeEnd)
        )
        ...
        test_reporters.SQLiteReporterTestCase.tstInsertSingleMinimizerResult(
            self, engine
        )
