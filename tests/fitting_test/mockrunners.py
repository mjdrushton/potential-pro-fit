...
class MockRunner1Runner(object):
  ...
  def createFromConfig(runnerName, fitRootPath, cfgitems):
    cdict = dict(cfgitems)
    return MockRunner1Runner(runnerName, cdict['remote_dir'], cdict['header_filename'], fitRootPath)
...
class MockRunner2Runner(object):
  ...
  def createFromConfig(runnerName, fitRootPath, cfgitems):
    cdict = dict(cfgitems)
    return MockRunner2Runner(runnerName, cdict['remote_dir'], int(cdict['ncpus']), fitRootPath)
