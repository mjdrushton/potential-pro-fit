...
class LocalRunnerTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        for i in range(12):
            ...
            os.mkdir(jd)
            jobs.append(self.jobfactory.createJob(jd, variables))
    ...
    def _testjob(self, runner, jobid):
        ...
        expect = [("runner_files", self.DIR), ("job_files", self.DIR)]
        ...
        expect = [("runjob", self.FILE), ("output", self.DIR)]
        ...
        common.logger.debug(
            "output.res: %s" % open(os.path.join(ddir, "output.res")).read()
        )
...
"""
        ...
        runner = pro_fit.runners.LocalRunner.createFromConfig(
            "RunnerName", self.tempd, parser.items("Runner:RunnerName")
        )
