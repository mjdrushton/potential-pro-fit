...
from ._runnercommon import (DIR, FILE, CheckPIDS, isdir_remote, runfixture,
                            runnertestjob)
...
def _createRunner(runfixture, vagrantbox, ncpu=1):
    ...
    if runfixture.remoted:
        runner = pro_fit.runners.RemoteRunner(
            "RemoteRunner",
            "ssh://%s@%s:%s/%s"
            % (username, hostname, port, runfixture.remoted),
            ncpu,
...
def testUrlParse():
    ...
    username, host, port, path = urlParse(
        "ssh://username@localhost/remote/path"
    )
...
def testTerminate(tmpdir_factory, runfixture, vagrant_basic):
        ...
        for i in range(5):
            ...
            for job in f.jobs:
                fullpath = posixpath.join(
                    job.remotePath, "job_files", "JOB_PID"
                )
                channel.send(fullpath)
                found = channel.receive()
                logger.debug(
                    "checkpaths: '%s' found = '%s'" % (fullpath, found)
                )
                count += found

            logger.debug(
                "checkpaths count on %d attempt: %d (expecting %d)",
                i,
                count,
                len(f.jobs),
            )
    ...
    for job in jobs1:
        ...
        assert not isdirchannel.receive(), (
            "Remote path should not exist but does: %s" % job.remotePath
        )

    # Check that the batch directory has been deleted.
    isdirchannel.send(b1_future.remoteBatchDir)
    assert not isdirchannel.receive(), (
        "Remote path should not exist but does: %s" % b1_future.remoteBatchDir
    )
...
def testClose(runfixture, vagrant_basic, tmpdir_factory):
    ...
    assert (
        1 + 1 + runner._inner._numDownload + runner._inner._numUpload
        == rstatus.numchannels
    )
    ...
    assert not isdirchannel.receive(), (
        "Batch 1 remote path should not exist but does: %s" % b1.remoteBatchDir
    )

    isdirchannel.send(b2.remoteBatchDir)
    assert not isdirchannel.receive(), (
        "Batch 2 remote path should not exist but does: %s" % b1.remoteBatchDir
    )

    isdirchannel.send(runner._inner._remotePath)
    assert not isdirchannel.receive(), (
        "Runner remote path should not exist but does: %s"
        % b1.remoteBatchDir
    )
    ...
    try:
        ...
        def waitForRun():
            ...
            while len(running) < ncpu:
                running = [
                    j
                    for j in b1.jobs
                    if not j.pidSetEvent is None and j.pidSetEvent.is_set()
                ]
                gevent.sleep(0.1)
            return running
        ...
        def waitForUpload():
            ...
            while len(uploaded) < len(b1_jobs) + len(b2_jobs):
                uploaded = [
                    j
                    for j in itertools.chain(b1.jobs, b2.jobs)
                    if "finish upload" in j.status
                ]
                gevent.sleep(0.1)
            return uploaded
