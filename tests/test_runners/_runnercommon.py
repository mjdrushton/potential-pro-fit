...
def runnertestjob(runfixture, jobid, expectstderr_stdout=False):
    ...
    common.logger.debug(
        "output.res: %s" % open(os.path.join(output_dir, "output.res")).read()
    )
...
def runfixture(tmpdir, request):
    ...
    for i in range(12):
        ...
        jd.mkdir()
        jobs.append(jobfactory.createJob(jd.strpath, variables))
    jobs = jobs
        ...
        for pid in pids:
            ...
            if status:
                ...
                if found == True:
                    ...
                    self.channel.waitclose()
                    assert False, (
                        "PID was found on remote host when it should no longer exist: %s"
                        % pid
                    )
