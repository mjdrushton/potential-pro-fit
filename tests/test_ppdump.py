...
def testGetColumnList():
    ...
    assert_that(columns).is_equal_to(ColumnKeysTestCase.evaluatorExpect())
    ...
    columns = ppdump._getColumnList(
        engine, [], [ppdump._EVALUATOR_COLUMN_SET, ppdump._VARIABLE_COLUMN_SET]
    )
    ...
    expect.extend(
        [
            v
            for v in ColumnKeysTestCase.variableExpect()
            if v != "variable:M_charge"
        ]
    )
    ...
    columns = ppdump._getColumnList(
        engine,
        ["variable:M_charge"],
        [ppdump._EVALUATOR_COLUMN_SET, ppdump._VARIABLE_COLUMN_SET],
    )
    assert_that(columns).is_equal_to(expect)
...
def testColumnSets():
    ...
    outputlines = _run_ppdump(
        ["-f %s" % _getdbpath(), "--fit-variable-columns"]
    )
    ...
    outputlines = _run_ppdump(
        ["-f %s" % _getdbpath(), "--evaluator-columns --variable-columns"]
    )
...
def testOptionColumn():
    ...
    colkeys = [
        "evaluator:T=1600K_Gd_x=0.1:T=1600K_Gd_x=0.1:Volume:V:extracted_value",
        "variable:M_charge",
    ]

    evaluator = [85314.6263020015, 85254.8501058408, 85183.2203962876]
...
def test_outputRGrid(tmpdir):
    ...
    outputlines = _run_ppdump(
        [
            ...
            "--gridx=variable:A",
            "--gridy=variable:B",
            "--gridz=evaluator:mult:mult:val:Z:extracted_value",
        ]
    )
