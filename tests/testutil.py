...
def assertFloatWithinPercentage(
    testCase, expect, actual, percenttolerance=0.5, places=5
):
  ...
  that actual is the same as expect for given number of places"""
    ...
    if expect == 0.0:
        ...
        percentDifference = math.fabs(
            ((actual - expect) / float(expect)) * 100.0
        )
        msg = (
            "Actual %f != Expect %f within tolerance of %f%% (difference = %f%%)"
            % (actual, expect, percenttolerance, percentDifference)
        )
...
def _compareCollection(
    ...
    if expectType == list or expectType == tuple:
        ...
        for i, (e, a) in enumerate(zip(expect, actual)):
            _compareCollection(
                path + "[%d]" % i, testCase, e, a, places, percenttolerance
            )
...
def compareCollection(
    testCase, expect, actual, places=5, percenttolerance=None
):
...
def _getVagrantDir():
    return os.path.join(os.path.dirname(__file__), "vagrant")
