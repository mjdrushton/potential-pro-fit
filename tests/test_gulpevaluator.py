...
class GulpDrvParserTestCase(unittest.TestCase):
    def setUp(self):
        self.filename = os.path.join(
            _getResourceDir(), "job_files", "output", "out.drv"
        )
...
class GulpDrvEvaluatorTestCase(unittest.TestCase):
    ...
    def testVectorMagnitude(self):
        ...
        with open(
            os.path.join(_getResourceDir(), "job_files", "drv.cfg")
        ) as infile:
            self.parser.read_file(infile)

            evaluator = pro_fit.evaluators.Gulp_DRVEvaluator.createFromConfig(
                "Gulp:DRV",
                _getResourceDir(),
...
class GulpEvaluatorTestCase(unittest.TestCase):
    ...
    def testElastic(self):
        ...
        evaluator = pro_fit.evaluators.GulpEvaluator.createFromConfig(
            "Gulp", _getResourceDir(), self.parser.items("Evaluator:Elastic")
        )
        ...
        testutil.compareCollection(
            self, fractionalextractedvalues, fractionaldiffs
        )
    ...
    def testBulkModulus(self):
        ...
        testutil.compareCollection(
            self,
            {
                "bulkmodulus_reuss": 232.60093,
                "bulkmodulus_voigt": 233.60093,
                "bulkmodulus_hill": 234.60093,
            },
            extractedvalues,
        )
        ...
        testutil.compareCollection(
            self,
            {
                "bulkmodulus_reuss": 0.06959628,
                "bulkmodulus_voigt": 0.01565621739,
                "bulkmodulus_hill": 0.07123712329,
            },
            fractionaldiffs,
        )
    ...
    def testShearModulus(self):
        ...
        testutil.compareCollection(
            self,
            {
                "shearmodulus_reuss": 133.19021,
                "shearmodulus_voigt": 139.56198,
                "shearmodulus_hill": 136.37610,
            },
            extractedvalues,
        )
    ...
    def testUnitCell(self):
        ...
        evaluator = pro_fit.evaluators.GulpEvaluator.createFromConfig(
            "Gulp", _getResourceDir(), self.parser.items("Evaluator:UnitCell")
        )
    ...
    def testEnergy(self):
        ...
        evaluator = pro_fit.evaluators.GulpEvaluator.createFromConfig(
            "Gulp", _getResourceDir(), self.parser.items("Evaluator:Energy")
        )

        job = pro_fit.jobfactories.Job(None, _getResourceDir(), None)
        actual = evaluator(job)
        actual = dict([(v.name, v.meritValue) for v in actual])
        expect = {"optimisation_penalty": 0.0, "lattice_energy": 69.87295}
        testutil.compareCollection(self, expect, actual)

        evaluator = pro_fit.evaluators.GulpEvaluator.createFromConfig(
            "Gulp", _getResourceDir(), self.parser.items("Evaluator:BadEnergy")
        )
        job = pro_fit.jobfactories.Job(None, _getResourceDir(), None)
        actual = evaluator(job)
        actual = dict([(v.name, v.meritValue) for v in actual])
    ...
    def testPhonon(self):
        ...
        evaluator = pro_fit.evaluators.GulpEvaluator.createFromConfig(
            "Gulp", _getResourceDir(), self.parser.items("Evaluator:Phonon")
        )
    ...
    def testEvaluationErrors(self):
        ...
        evaluator = pro_fit.evaluators.GulpEvaluator.createFromConfig(
            "Gulp", _getResourceDir(), self.parser.items("Evaluator:Error")
        )
