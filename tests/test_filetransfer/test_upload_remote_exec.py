...
def testKeepAlive(tmpdir, execnet_gw, channel_id):
    ...
    try:
        ...
        ch1.send(
            {
                ...
                "remote_path": path.strpath,
            }
        )
        msg = ch1.receive(10.0)
        assert_that(msg).is_equal_to(
            dict(msg="READY", channel_id=channel_id, remote_path=path.strpath)
        )
        ...
        msg = ch1.receive(10.0)
        assert_that(msg).is_equal_to(
            {"msg": "KEEP_ALIVE", "channel_id": channel_id, "id": "1234"}
        )
    finally:
        ch1.send(None)
        ch1.waitclose(5)
...
def testSendFile(tmpdir, execnet_gw, channel_id):
    ...
    try:
        ...
        ch1.send(dict(msg="START_UPLOAD_CHANNEL", remote_path=destdir.strpath))
...
def testMkfile(tmpdir, execnet_gw, channel_id):
    ...
    try:
        ...
        ch1.send(dict(msg="MKDIR", remote_path="path1", id=fileid))
        msg = ch1.receive(10.0)
        assert msg == dict(
            msg="MKDIR",
            id=fileid,
            channel_id=channel_id,
            remote_path=path1.strpath,
        )
        ...
        ch1.send(
            dict(msg="MKDIR", remote_path=path2.strpath, mode=0o600, id=fileid)
        )
        msg = ch1.receive(10.0)
        assert msg == dict(
            msg="MKDIR",
...
def testMkdirs(tmpdir, execnet_gw, channel_id):
    ...
    ch1 = execnet_gw.remote_exec(file_transfer_remote_exec)
    try:
        ch1.send(dict(msg="START_UPLOAD_CHANNEL", remote_path=dest.strpath))
        ...
        ch1.send(
            dict(
                msg="MKDIRS",
                remote_path=three_deep.strpath,
                id=transid,
                mode=0o700,
            )
        )
        ...
        assert (
            stat.S_IMODE(os.stat(dest.join("one", "two").strpath).st_mode)
            == 0o700
        )
        assert (
            stat.S_IMODE(
                os.stat(dest.join("one", "two", "three").strpath).st_mode
            )
            == 0o700
        )
