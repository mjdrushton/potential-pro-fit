...
def testLockTree_2():
    ...
    lt.add("two/a/c")
    assert set(
        ["two", "two/three", "two/three/four", "two/a/b", "two/a/c"]
    ) == set(lt.locked())
...
def testLockTree():
    ...
    assert (
        lt.rootnode.children["one"].children["two"].children["three"].islocked
        is None
    )
    assert (
        lt.rootnode.children["one"]
        .children["two"]
        .children["three"]
        .children["four"]
        .islocked
    ...
    assert set([]) == set(lt.unlocked())
    assert set(["one/two/three/four", "one", "one/two/file.txt"]) == set(
        lt.locked()
    )
    ...
    assert set(["/root/directory/one/two/three/four"]) == set(
        lt.unlocked(include_root=True)
    )
    assert set(
        ["/root/directory/one", "/root/directory/one/two/file.txt"]
    ) == set(lt.locked(include_root=True))

    lt.unlock("one/two/file.txt")
    assert set(["one", "one/two/file.txt", "one/two/three/four"]) == set(
        lt.unlocked()
    )
...
def testLockTree_get():
    ...
    assert id(
        lt.rootnode.children["one"].children["two"].children["three"]
    ) == id(lt["one/two/three"])

    assert id(lt.rootnode.children["one"]) == id(
        lt[posixpath.join(rootdir, "one")]
    )
    assert id(lt.rootnode.children["one"].children["two"]) == id(
        lt[posixpath.join(rootdir, "one/two")]
    )
    assert id(
        lt.rootnode.children["one"].children["two"].children["three"]
    ) == id(lt[posixpath.join(rootdir, "one/two/three")])
